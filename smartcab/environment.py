...
# Do an output directory
output_text = os.path.dirname(__file__)
path = os.path.join(output_text, 'output_qlearn2.txt')
        ...
        if self.primary_agent is not None:
            ...
            if agent_deadline <= self.hard_time_limit:
                ...
                with open(path, 'a') as file:
                    file.write("\nPrimary agent hit hard time limit.")
            elif self.enforce_deadline and agent_deadline <= 0:
                self.done = True
                print "Environment.step(): Primary agent ran out of time! Trial aborted."
                with open(path, 'a') as file:
                    file.write("\nPrimary agent ran out of time!")
    ...
    def act(self, agent, action):
        ...
        if agent is self.primary_agent:
            if state['location'] == state['destination']:
                ...
                with open(path, 'a') as file:
                    file.write("\nPrimary agent has reached destination")
